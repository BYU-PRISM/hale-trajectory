...
def integrateSS_wrapper_wind(config):
    ...
    # Calculate initial direction, opposite of the wind.
    w_norm = w/np.linalg.norm(w)
    heading_vector_new = -w_norm
    psi_new = (np.arctan2(heading_vector_new[1],heading_vector_new[0]) + 2*np.pi)%(2*np.pi)
    w_angle = np.arctan2(w_norm[1],w_norm[0])
    orth_w_angle = w_angle + np.pi/2
    orth_w_x = np.cos(orth_w_angle)
    orth_w_y = np.sin(orth_w_angle)
    x_new = orth_w_x * config['trajectory']['x']['max']
    y_new = orth_w_y * config['trajectory']['y']['max']
    
    # Load back into config file
    config['trajectory']['x']['initial_value'] = x_new
    config['trajectory']['y']['initial_value'] = y_new
    config['trajectory']['psi']['initial_value'] = psi_new
    ...
    # Initial Conditions
    v_a_0 = v_a_min[0] # 43.994 # Initial Velocity (m)
...
#        intermediates = pd.concat([intermediates, model_output_df])
    solData['time'] = t
    model_outputs = [pd.Series(uavDynamicsWrapper_wind(SV[1],[],MV,h_0,[],w,smartsData,config_file,5)).to_frame() for SV in solData.iterrows()]
    ...
    solDataOut = solData[['time', 'flux', 't','zenith','azimuth','sn1','sn2','sn3','sun_h','w_n','w_e','w_d']]
