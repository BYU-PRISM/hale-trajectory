...
def plotTotalEnergy_multiple(folderList):
    ...
    #        print(files)
            ...
            # Plots
            plt.plot(df_opt.time_hr,df_opt.te_kwh,label='Opt '+' '.join(name.split()[2:]),color=color)
            plt.plot(df_ss.time_hr,df_ss.te_kwh,'--',label='SS '+' '.join(name.split()[2:]),color=color)
            print(name)
...
def plotSOC_multiple(folderList):
    ...
    #        print(files)
            ...
            # Plots
            plt.plot(df_opt.time_hr,df_opt.soc,label='Opt '+' '.join(name.split()[2:]),color=color)
            plt.plot(df_ss.time_hr,df_ss.soc,'--',label='SS '+' '.join(name.split()[2:]),color=color)
            print(name)
...
def plotAltitude_multiple(folderList):
    ...
    for folder in folderList:
        ...
        if(name.split()[0]=='SM'):
            ...
            df_ss = fix_units(df_ss)
            # Plots
            plt.plot(df_opt.time_hr,df_opt.h,label='Opt '+' '.join(name.split()[2:]),color=color)
            plt.plot(df_ss.time_hr,df_ss.h,'--',label='SS '+' '.join(name.split()[2:]),color=color)
            print(name)
...
def plotTotalEnergy_multiple2(folderList,case_type='dT_loc'):
    fig = plt.figure()
    # Colors
    palette = itertools.cycle(sns.color_palette())
    colors = {'140':next(palette),'130':next(palette),'120':next(palette),'110':next(palette),'100':next(palette),'90':next(palette)}
    ...
    for folder in folderList:
        # Load optimized data
        name = ' '.join(folder.split()[2:]).split('/')[0]
        # Extract battery mass from folder name
        bat_mass = name.split()[-2]
        color = colors[bat_mass]
        files = glob.glob('./Data/'+folder+'/sim/' + case_type + '/sim_results*.csv')
        df = pd.read_csv(files[-1])
        df = fix_units2(df)
        if(name.split()[0]=='SM'):
            # Plots
            plt.plot(df.time_hr,df.ss_te_kwh,':',label=name,color=color)
        else:
            # Plots
            plt.plot(df.time_hr,df.opt_te_kwh,label='Opt '+' '.join(name.split()[2:]),color=color)
            plt.plot(df.time_hr,df.ss_te_kwh,'--',label='SS '+' '.join(name.split()[2:]),color=color)
            print(name)
            print('Opt max: ' + str(df['opt_te_kwh'].max()))
            print('SS max: ' + str(df['ss_te_kwh'].max()))
    plt.legend(ncol=2,fontsize=11)
    plt.xlim([0,24])
    plt.ylim([-10,60])
    plt.xlabel('Time (hr)')
    plt.ylabel('Total Energy (kWh)')
    plt.title('Total Energy vs Battery Mass (' + case_type +' simulation)')
    
def plotSOC_multiple2(folderList,case_type='dT_loc'):
    fig = plt.figure()
    df_bar = pd.DataFrame(columns=['Battery Mass','SOC','Trajectory'])
    # Colors
    palette = itertools.cycle(sns.color_palette())
    ...
    for folder in folderList:
        ...
        if(name.split()[0]=='SM'):
            # Plots
            plt.plot(df.time_hr,df['SS SOC'],':',label=name,color=color)
            # Bar plot
            df_bar = df_bar.append({'Battery Mass':int(bat_mass),'SOC':df['SS SOC'].values[-1],'Trajectory':'SM'},ignore_index=True)
        else:
            # Plots
            plt.plot(df.time_hr,df['OPT SOC'],label='Opt '+' '.join(name.split()[2:]),color=color)
            plt.plot(df.time_hr,df['SS SOC'],'--',label='SS '+' '.join(name.split()[2:]),color=color)
            print('Opt Final: ' + str(df['OPT SOC'].values[-1]))
            print('SS Final: ' + str(df['SS SOC'].values[-1]))
            # Bar plot
            df_bar = df_bar.append({'Battery Mass':int(bat_mass),'SOC':df['SS SOC'].values[-1],'Trajectory':'SS'},ignore_index=True)
            df_bar = df_bar.append({'Battery Mass':int(bat_mass),'SOC':df['OPT SOC'].values[-1],'Trajectory':'Opt'},ignore_index=True)
    plt.legend(ncol=2,fontsize=11)
    plt.xlim([0,24])
    plt.ylim([-0.205,1.05])
    plt.xlabel('Time (hr)')
    plt.ylabel('State of Charge')
    plt.title('State of Charge vs Battery Mass (' + case_type +' simulation)')
    # Bar chart
#    fig = plt.figure()
#    df_bar.plot.bar()
    sns.set_style("whitegrid")
...
def fix_units2(df):
    df['time_hr'] = df['Time']/3600
    df['ss_te_kwh'] = df['SS Total Energy']
    df['opt_te_kwh'] = df['OPT Total Energy']
    return df
...
##    plotHourly(data,2)
    ...
    folderList2 = ['hale_2018_01_25_15_42_06 - Winter Battery 140 kg/sim_results_02_08',
                  #'hale_2018_02_01_11_01_18 - SM 140 kg',
                  'hale_2018_01_26_12_07_49 - Winter Battery 130 kg/sim_results_02_08',
                  'hale_2018_01_29_15_13_05 - SM 130 kg/sim_results_02_07',
                  'hale_2018_01_26_12_08_16 - Winter Battery 120 kg/sim_results_02_08',
                  'hale_2018_01_29_14_01_01 - SM 120 kg/sim_results_02_13',
                  #'hale_2018_01_26_12_08_37 - Winter Battery 110 kg',
                  'hale_2018_01_29_13_49_29 - SM 110 kg/sim_results_02_13',
                  #'hale_2018_01_26_12_08_55 - Winter Battery 100 kg',
                  'hale_2018_01_29_13_36_29 - SM 100 kg/sim_results_02_13',
                  #'hale_2018_01_26_12_32_22 - Winter Battery 90 kg',
                  #'hale_2018_02_02_12_01_31 - SM 90 kg'
                  ]
    plotTotalEnergy_multiple2(folderList2,case_type='dT_loc')
    plotSOC_multiple2(folderList2,case_type='dT_loc')
    # case_type: 
        # use dT_loc for varying temperature, local flux results
        # use dT_avg for varying temperature, average flux results
        # use T_loc for constant temperature, local flux results
        # use T_avg for constant temperature, average flux results
