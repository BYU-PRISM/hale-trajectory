...
def new_objective(xv,h_0,w,smartsData,config_file):
    x0 = xv[:3]
    v = xv[3]
#    d = uavDynamicsWrapper(x0,[],[],h_0,v,smartsData,config_file,2)
    P_N = uavDynamicsWrapper_wind(x0,[],[],h_0,v,w,smartsData,config_file,3)
    return P_N
...
def new_constraints(xv,h_0,w,smartsData,config_file):
    x0 = xv[:3]
    v = xv[3]
    d = uavDynamicsWrapper_wind(x0,[],[],h_0,v,w,smartsData,config_file,2)
    return d
#
...
def findSteadyState(x0,h_0,smartsData,config_file):
    ...
    cons = ({'type': 'eq', 'fun':new_constraints,'args':(h_0,w,smartsData,config_file)})
    ...
    sol = minimize(new_objective,
                   [x0v0],
                   args=(h_0,w,smartsData,config_file),
                   bounds = bounds,
                   constraints=cons,
                   method='SLSQP',
...
#                   options={'disp':True,'eps':1e-8,'ftol':1e-10})
    ...
    phimin = sol.x[2]
    vmin = sol.x[3]
    clmin = uavDynamicsWrapper_wind(sol.x[:3],[],[],h_0,vmin,w,smartsData,config_file,4)
    pmin = sol.fun
#    vmin = sol.x
